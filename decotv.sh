...
# ---- 自动探测 core 容器内部端口（优先 ExposedPorts，否则默认 3000）----
...
# ✅ 关键：nginx 在 docker 里，不能 proxy_pass 127.0.0.1:宿主机端口（容易 502）
# 正确做法：让 nginx 加入同一个网络，proxy_pass 到服务名 decotv-core:内部端口
write_nginx_conf(){
...
run_nginx_container(){
  # 确保 network 存在（compose up 后一般就有）
...
has_local_cert(){
  local domain="$1" cd; cd="$(acme_cert_dir "$domain")"
...
绑定域名反代(){
  ...
  # 写入反代域名（用于状态/输出优先显示域名）
  # 先删旧的 RP_ 键，再追加新的
  sed -i '/^RP_DOMAIN=/d' "$ENVF" 2>/dev/null || true
...
菜单(){
  ...
  if rp_enabled; then
    echo "域名反代：已启用（${NGX_C}）｜ 域名：$(rp_domain)"
  else
    echo "域名反代：未启用"
  fi
  echo
